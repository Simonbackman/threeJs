function init() {
  ...
  var sun = sunSphere(2.05);
  var moon = moonSphere(2.05);
  ...
  gui.add(pointLight.position, 'y', 0, 15);
  ...
  var geometry_back2 = new THREE.BoxGeometry(0.1, 4.5, 6);
  var ground_back2 = new THREE.Mesh(geometry_back2, material_grass);
  ground_back2.position.set(7.6, 2, -4.55);
  scene.add(ground_back2);
  ...
  //a ground under house

  var geometry_right = new THREE.BoxGeometry(15, 0.6, 9.7);
  var baseGround = new THREE.Mesh(geometry_right, material_grass);
  baseGround.position.set(-0.1, -0.4, 3.5);
  ...
  loader.load('./house/scene.gltf', gltfHouse => {
    ...
    // gltfHouse.scene.castShadow = true;
    scene.add(gltfHouse.scene);
  });

  loader.load('./rubber_duck/scene.gltf', gltfDuck => {
    gltfDuck.scene.position.set(0, 0.7, -4.5);
    gltfDuck.scene.rotation.y = 4.7;
    scene.add(gltfDuck.scene);

    gui.add(gltfDuck.scene.position, 'x', -3, 3);
    gui.add(gltfDuck.scene.rotation, 'y', 1.4, 4.7);
  });

  loader.load('./hanging_ivy/scene.gltf', gltfIvy => {
    gltfIvy.scene.position.set(4.9, -1.2, -0.6);
    gltfIvy.scene.scale.set(0.009, 0.019, 0.03);
    scene.add(gltfIvy.scene);
  });

  // loader.load('./house2/scene.gltf', gltfTH => {
  //   scene.add(gltfTH.scene);
  // });
  ...
  var renderM = () => {
    ...
    t += 0.005;
    sun.position.x = 2 * Math.cos(t) + 0;
    sun.position.z = 2 * Math.sin(t) + 0;

    moon.position.x = -20 * Math.cos(t) + 0;
    moon.position.z = -20 * Math.sin(t) + 0;

    pointLight.position.x = 20 * Math.cos(t) + 0;
    pointLight.position.z = 20 * Math.sin(t) + 0;

    // waterfall drops animation
    if (count % 3 == 0) {
      for (var i = 0; i < 5; i++) {
        drops.push(new Drop());
      }
...
function moonSphere(size) {
  var textureMoon = new THREE.TextureLoader().load('./textures/moon.jpg');
  var geometry = new THREE.SphereGeometry(size, 20, 20);
  var material = new THREE.MeshPhongMaterial({
    // color: 'white',
    map: textureMoon
